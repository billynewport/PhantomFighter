...
void StartRandomExplosion(x,y,range)
WORD x,y,range;
{
   register int offx,offy;
   offx=rnd(range); offy=rnd(range);
   if(rnd(1000)>500)offx=-offx;
   if(rnd(1000)>500)offy-=offy;
   SysMakeExplosion(NULL,NULL,x+offx,y+offy,0,0,&ExplosionImage[0],3);
}
...
int a;
{
   Move(80*a,7);
   SetAPen(7);
   Text(bm_bottom1,TokenNames[a],8);
}

void PrintTokensOut()
{
   PrintToken(TOK_FASTER);
   PrintToken(TOK_LASER);
   PrintToken(TOK_DOUBLE);
   PrintToken(TOK_PLASMA);
   PrintToken(TOK_SIDE);
   PrintToken(TOK_HOMER);
}
